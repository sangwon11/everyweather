  ...
  useLayoutEffect(() => {
    const handleScroll = () => {
      ...
      buttons.forEach(button => {
        ...
        const extraTranslateX = isVeryCenter ? 50 : 0; // 중앙에 있을 때 추가로 30px 오른쪽으로 이동
        const scaleTransform = isVeryCenter ? ' scale(1.5)' : (distanceFromCenter < 40 ? ' scale(1.5)' : ' scale(1.0)');
